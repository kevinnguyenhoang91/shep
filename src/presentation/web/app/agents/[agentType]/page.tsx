...
import { getAgentGraphDescriptor } from '@/components/agent-editor/agent-graph-descriptors';
...
export default async function AgentEditorRoute({ params }: RouteProps) {
  ...
  const prompts = await listPrompts.execute({ agentType });

  if (prompts.length === 0) {
    notFound();
  }

  const graph = getAgentGraphDescriptor(agentType);
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 p-6">
      ...
      <AgentEditorTabs agentType={agentType} prompts={prompts} graph={graph} />
    </div>
  );
}
