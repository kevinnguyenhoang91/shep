...
describe('GetSupervisorPolicyUseCase', () => {
  ...
  it('returns null when no policy exists for the scope', async () => {
    const policy = await getPolicy.execute({ appId: 'app-missing' });
  ...
  it('returns the app-scoped policy when only an app row exists', async () => {
    ...
    const policy = await getPolicy.execute({ appId: 'app-1' });
  ...
  it('falls back to the app-scoped policy when no feature override exists', async () => {
    ...
    const policy = await getPolicy.execute({ appId: 'app-1', featureId: 'feat-missing' });
  ...
  it('returns the feature-scoped override when one exists', async () => {
    ...
    const policy = await getPolicy.execute({ appId: 'app-1', featureId: 'feat-7' });
  ...
  it('returns null when the feature-scoped lookup is for an app with no rows', async () => {
    ...
    const policy = await getPolicy.execute({ appId: 'app-2', featureId: 'feat-7' });
    expect(policy).toBeNull();
  });

  it('throws when appId is missing (NFR-7 scope isolation)', async () => {
    await expect(getPolicy.execute({ appId: '' as unknown as string })).rejects.toThrow();
  });
});
